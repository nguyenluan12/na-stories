...
export default function ListenAndRead() {
  const sentences = [
    {
      id: 1,
      person: "kid",
  ...
  return (
    <div className="flex items-center flex-col p-20  w-screen h-screen">
      ...
      <div className="fixed bottom-0 flex justify-center border-t-2 rounded-2xl w-full h-13 bg-white">
        <button
          onClick={() => {
            handleAddSentence();
            scrollToBottom(items.length - 1);
          }}
          className="w-40 h-full p-1 px-4 my-2 border-2 border-green-500 rounded-lg bg-green-500 text-white font-semibold text-lg shadow-lg transition-transform duration-300 hover:shadow-xl hover:scale-105 active:scale-95"
        >
          Next
        </button>
      </div>
